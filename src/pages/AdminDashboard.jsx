...
const AdminDashboard = () => {
  ...
  const handleDownload = async (filePath) => {
    try {
      ...
      console.error(err);
      alert("Download failed. Please check if the file exists on the server.");
    }
  };
                ...
                <tbody>
                  {orders.map((o) => (
                    <tr key={o._id}>
                      ...
                      <td>{o.paymentStatus || "Pending"}</td>
                      <td>
                        <span style={{ fontWeight: "bold", color: o.status === "Accepted" ? "green" : o.status === "Rejected" ? "red" : "orange" }}>
                          {o.status || "Pending"}
                        </span>
                      </td>
                      <td className="text-nowrap">
                        {!o.status || o.status === "Pending" ? (
                          ...
                          <Button size="sm" variant="danger" onClick={async () => {
                            if (!window.confirm("Delete this order?")) return;
                            try { await API.delete(`/admin/order/${o._id}`); loadOrders(); } catch { alert("Delete failed"); }
                          }}>Delete</Button>
                        )}
                      </td>
                    </tr>
                ...
                <tbody>
                  {messages.map((msg) => (
                    <tr key={msg._id}>
                      ...
                      <td>
                        {msg.prescription ? (
                          <button className="btn btn-sm btn-primary" onClick={() => handleDownload(msg.prescription)}>Download</button>
                        ) : "-"}
                      </td>
                      <td>
                        {msg.brokenFrame ? (
                          <button className="btn btn-sm btn-primary" onClick={() => handleDownload(msg.brokenFrame)}>Download</button>
                        ) : "-"}
                      </td>
                      <td>{new Date(msg.createdAt).toLocaleString()}</td>
                      <td>
                        <button className="btn btn-danger btn-sm" onClick={() => deleteMessage(msg._id)}>Delete</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <>
            ...
            {profile && (
              <Form onSubmit={updateProfile} style={{ maxWidth: '500px' }}>
                ...
                <Form.Group className="mb-2">
                  <Form.Label>Password</Form.Label>
                  <Form.Control type="password" placeholder="Enter new password" value={profile.password || ""} onChange={(e) => setProfile({ ...profile, password: e.target.value })} />
                </Form.Group>
                <Button type="submit" variant="dark">Save Changes</Button>
              </Form>
            )}
