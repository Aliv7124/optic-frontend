...
const SunglassesPage = () => {
  ...
  const loadProducts = async () => {
    try {
      setLoading(true);
      const res = await API.get("/products?category=sunglasses");
      setProducts(res.data.data);
    } catch (err) {
      console.error(err);
      alert("Failed to load sunglasses");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Container className="mt-4">
      <h3 className="mb-4">Sunglasses</h3>
          ...
          {products.map((p) => (
            <Col key={p._id}>
              <Card
                ...
                onClick={(e) => {
                  // Navigate to product details if not clicking Buy Now
                  {
          ...
          <Row xs={1} sm={2} md={3} lg={4} className="g-4">
            {products.map((p) => (
              <Col key={p._id}>
                ...
                >
                  {p.image && (
                    <div style={{ overflow: "hidden", borderRadius: "18px 18px 0 0" }}>
                      <Card.Img
                        src={`http://localhost:5001${p.image}`}
                        alt={p.name}
                        style={{
